...
Modified as part of the Parapipe pipeline to take a list of gffs and assemblies,
which it will auto-pair by basename. The original script can be found at
github.com/ArthurVM/gff2protein

Usage:
    gff3_to_proteins.py --gff <gff_1> <gff_2> <...> <gff_n> --fasta <fasta_1> <fasta_2> <...> <fasta_n> --featureID <featureID>
...
def pairFiles(gff_list, fasta_list):
    """ Takes a list of gff files and a list of fasta files and pairs them according
    to their basename. NOTE: if basenames differ at all, this will fail.
    """

    if len(gff_list) != len(fasta_list):
        print("WARNING! The number of gffs and fastas provided are not equal!\nContinuing with due care and pessimism...")
...
def parseArgs(argv):
    ...
    parser.add_argument('-g', '--gff', type=str, nargs='*', help='GFF files containing gene features.')
    parser.add_argument('-f', '--fasta', type=str, nargs='*', help='Nucleotide sequence files in FASTA format')
...
def main(args):
    ...
    for gff, fasta in pairFiles(args.gff, args.fasta):
        print(f"Running on pair {gff} {fasta}...")
        fa_rec = pyfaidx.Fasta(fasta)

        prot_file = "{base}-prot.fa".format(base=os.path.splitext(gff)[0])
        cdna_file = "{base}-cdna.fa".format(base=os.path.splitext(gff)[0])

        # gff_file = cds_to_exon(gff_file)
        extract_recs(gff, fa_rec, args.featureID, prot_file, cdna_file)
