...
def make_plots(json_data):
    ...
    def make_map_table(json_data):
        ...
        for sample, report in json_data.items():
            ...
            ordered_list.append([ sample,\
             ...
             het_k, \
             het_bp, \
             f"{al_stats['num_snps']} ({len(al_stats['unique_snps'])})"])
    ...
    def make_st_table(json_data):
        ...
        for sample, report in json_data.items():
            for scheme, data in report["st_stats"]["schemes"].items():
        ...
        for scheme, data in st_stats.items():
            ...
            k = list(data.keys())
            table = [["Sample", *[locus for locus, cov in data[k[0]].items()]]]

            for sample, loci_cov in data.items():
                row = [sample, *[f"{np.round(float(cov['BOC']), 3)} ({np.round(float(cov['DOC']), 3)}x)" for locus, cov in loci_cov.items()]]
                table.append(row)
    ...
    st_plots = []
    for st, data in json_data[list(json_data.keys())[0]]["sequence_phylo"].items():
...
def main(argv):
    ...
    pdf.print_page(["allele_matrix.png"])
