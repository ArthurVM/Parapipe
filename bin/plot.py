...
def _plot_non_clustered(    baf_matrix : pd.DataFrame,
    ...
    x_min = df_sorted['x_axis'].min()
    x_max = df_sorted['x_axis'].max()
    af_jp.ax_marg_x.set_xlim(x_min, x_max)
    af_jp.ax_joint.set_xlim(x_min, x_max)
...
def _preprocess_for_plotting(baf_matrix: pd.DataFrame):
    df_sorted = baf_matrix.sort_values(by=['chromosome', 'position'])

    # Calculate chromosome sizes by genomic span
    chrom_sizes = (
        df_sorted.groupby('chromosome')['position']
        .agg(lambda x: x.max() - x.min())
        .sort_values(ascending=False)
    )
    unique_chromosomes = chrom_sizes.index.tolist()
    ...
    padding = int(max_pos * 0.01)  # 1% of max position as padding
    ...
    for chrom in unique_chromosomes:
        ...
        midpoint = current_offset + (max_pos - min_pos) / 2
        chrom_ticks[chrom] = midpoint

        for pos in chrom_data['position']:
            x_coords.append(current_offset + (pos - min_pos))

        current_offset += (max_pos - min_pos) + padding

    df_sorted['x_axis'] = x_coords
    return df_sorted, chrom_ticks
